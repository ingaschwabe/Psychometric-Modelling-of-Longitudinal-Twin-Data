#============================================================================#
# Following JAGS script fits a longitudinal AE twin model using sum-scores
...
model{
    ...
    for (fam in 1:n_mz){
        ...
        data_mz_twin1[fam,1:T] ~ dmnorm(mu[1:T] + a1_mz[fam,1:T], tau_e[1:T,1:T])
        data_mz_twin2[fam,1:T] ~ dmnorm(mu[1:T] + a1_mz[fam,1:T], tau_e[1:T,1:T])
    }
    ...
    for (fam in 1:n_dz){
        ...
        data_dz_twin1[fam,1:T] ~ dmnorm(mu[1:T] + a2_dz[fam,1:T] + 
                                        a4_dz_twin1[fam,1:T], tau_e[1:T,1:T])
        data_dz_twin2[fam,1:T] ~ dmnorm(mu[1:T] + a2_dz[fam,1:T] + 
                                        a4_dz_twin2[fam,1:T], tau_e[1:T,1:T])        
    }
